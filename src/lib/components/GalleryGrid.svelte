<script lang="ts">
	...
	type GenerationData = {
		items: Array<{
			...
			createdAt?: Date;
			updatedAt?: Date;
			steps: Array<{ id: number; artifacts: Array<{ id: number }> }>;
		}>;
		count: number;
	};

	interface Props {
		query: Promise<GenerationData> & { refresh?: () => void };
		perPage?: number;
		currentPage: number;
	...
	function getPreviewUrl(gen: GenerationData['items'][number]): string | null {
	...
	const columns = $derived(data ? distributeToColumns(data.items, columnCount) : []);
</script>
						...
						{#each column as gen (gen.id)}
							...
							<a href="/{gen.id}" class="group block">
								<Card.Root
									...
									{#if previewUrl}
										<img src={previewUrl} alt={gen.title} class="w-full h-auto" loading="lazy" />
									{:else}
		...
		<div class="flex items-center justify-center py-20">
			<Card.Root class="max-w-sm border-destructive">
				<Card.Header class="text-center py-3">
					...
					<Card.Description class="text-xs">{err.message}</Card.Description>
				</Card.Header>
				<Card.Footer class="justify-center py-2">
					<button onclick={reset} class="text-sm text-primary hover:underline">Retry</button>
				</Card.Footer>
			</Card.Root>
		</div>
	{/snippet}
