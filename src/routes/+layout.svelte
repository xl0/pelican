<script lang="ts">
	...
	// Display text for pending files button
	const pendingFilesText = $derived(
		app.pendingInputFiles.length === 0
			? 'Add reference'
			: app.pendingInputFiles.length === 1
				? app.pendingInputFiles[0].name
				: `${app.pendingInputFiles.length} files`
	);
</script>
						...
						<div class="space-y-1.5">
							<div class="flex items-center gap-2">
								...
								<Button
									...
									<span class="text-xs">{pendingFilesText}</span>
								</Button>
							</div>
							<Textarea
								id="prompt"
