<script lang="ts">
	...
	import { ChevronDown, CircleAlert, ImagePlus, WandSparkles } from '@lucide/svelte';
	...
	// let selectedStepIndex = $state<number>();
	// let referenceImageFile = $state(null as File | null);
	// let streamingContent = $state('');

	// Derived state from projectState.generations
	// const currentGeneration = $derived(getSelectedGeneration());

	// const generatedImage = $derived.by(() => {
	//   if (uiState.isGenerating && uiState.streamingContent) {
	//     // During streaming, show the streaming content
	//     if (outputFormat.current === 'svg') {
	//       const parts = uiState.streamingContent.split(/```(?:xml|svg)?\n<svg/);
	//       if (parts.length > 1) {
	//         let partialSvg = '<svg' + (parts[1].split('```')[0] || parts[1]);
	//         if (!partialSvg.includes('</svg>')) partialSvg += '</svg>';
	//         return `data:image/svg+xml;charset=utf-8,${encodeURIComponent(partialSvg)}`;
	//       }
	//     } else {
	//       return uiState.streamingContent.replace(/^```(?:ascii)?\n/, '');
	//     }
	//   }
	//   // Show completed generation's artifact
	//   const gen = currentGeneration;
	//   if (!gen || !gen.artifactPath) return null;
	//   // For completed generations, construct data URI from artifact path or use renderedImagePath
	//   if (gen.rawOutput) {
	//     if (outputFormat.current === 'svg') {
	//       const match = gen.rawOutput.match(/```(?:xml|svg)?\n(<svg[\s\S]*?<\/svg>)/);
	//       if (match) return `data:image/svg+xml;charset=utf-8,${encodeURIComponent(match[1])}`;
	//     } else {
	//       return gen.rawOutput.replace(/^```(?:ascii)?\n/, '').replace(/```$/, '');
	//     }
	//   }
	//   return null;
	// });
	...
	function handleFileSelect(event: Event) {
		...
		if (input.files && input.files[0]) {
			app.referenceImageFile = input.files[0];
		}
	}
</script>
	...
	<div class="mx-auto max-w-7xl space-y-3">
		...
		<div class="grid grid-cols-1 gap-0 lg:grid-cols-12">
			...
			<div class="lg:col-span-5 lg:border-r border-border lg:pr-3">
				<div class="space-y-3 p-3">
					...
					<div class="space-y-1.5">
						<div class="flex items-center gap-2">
							...
							<Button variant="ghost" size="sm" class="h-6 px-2 gap-1" onclick={() => document.getElementById('ref-image-input')?.click()}>
								...
								<span class="text-xs">{app.referenceImageFile ? app.referenceImageFile.name : 'Add reference'}</span>
							</Button>
						</div>
						<Textarea
							id="prompt"
					...
					<Collapsible.Root bind:open={promptTemplatesOpen}>
						<Collapsible.Trigger class="flex items-center justify-between w-full py-1.5 px-2 bg-muted hover:bg-muted/80 transition-colors">
			...
			<div class="lg:col-span-7">
				prompt = {app.renderedPrompt}
				refinement = {app.renderedRefinementPrompt}
